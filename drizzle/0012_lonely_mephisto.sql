ALTER TABLE `motivosIniciales` RENAME COLUMN "creadoPorId" TO "creado_por_id";--> statement-breakpoint
ALTER TABLE `motivosIniciales` RENAME COLUMN "userMedicoId" TO "medico_id";--> statement-breakpoint
...
ALTER TABLE `motivosIniciales` ALTER COLUMN "creado_por_id" TO "creado_por_id" text REFERENCES users(id) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `motivosIniciales` ALTER COLUMN "medico_id" TO "medico_id" text REFERENCES users(id) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `atenciones` ADD `ultimaModificacionPorId` text REFERENCES users(id);--> statement-breakpoint
ALTER TABLE `medicamentos` ADD `updateUserId` text REFERENCES users(id);--> statement-breakpoint
ALTER TABLE `medicamentos` ADD `estado` text DEFAULT 'activo';