CREATE TABLE `atencionAmendments` (
	...
	`contenidoCorregido` text NOT NULL,
	...
	FOREIGN KEY (`userIdMedico`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE TABLE `ordenesEstudio` (
	`id` text PRIMARY KEY NOT NULL,
	`atencionId` text NOT NULL,
	...
	`updated_at` integer,
	`deleted_at` integer,
	`diagnosticoPresuntivo` text NOT NULL,
	`estudiosSolicitados` text NOT NULL,
	...
	FOREIGN KEY (`userMedicoId`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
CREATE TABLE `preferenciaPerfilUser` (
	`id` text PRIMARY KEY NOT NULL,
	`userId` text NOT NULL,
	`nombrePerfil` text,
	`especialidad` text,
	`estado` text,
	`preferencias` text DEFAULT '{"configuracionGeneral":{"tema":"claro","idioma":"es","mostrarHistorialCompleto":true,"notificaciones":{"recordatorios":true,"alertasCriticas":true}},"signosVitales":{"mostrar":true,"campos":["peso","talla","temperatura","perimetroCefalico","presionSistolica","presionDiastolica","saturacionOxigeno","frecuenciaRespiratoria","perimetroAbdominal","imc","glucosa","dolor"]},"consulta":{"motivoInicial":true,"sintomas":true,"diagnostico":true,"tratamientoFarmacologico":true,"tratamientoNoFarmacologico":true,"planASeguir":true,"archivosAdjuntos":true,"notasPrivadas":false},"reportes":{"incluirDatosPaciente":true,"incluirDatosMedico":true,"incluirFirmaDigital":true}}',
	`created_at` integer DEFAULT (strftime('%s','now')) NOT NULL,
	`updated_at` integer,
	`deleted_at` integer,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `preferenciaPerfilUser_userId_nombrePerfil_unique` ON `preferenciaPerfilUser` (`userId`,`nombrePerfil`);--> statement-breakpoint
...
DROP INDEX "preferenciaPerfilUser_userId_nombrePerfil_unique";--> statement-breakpoint
...
ALTER TABLE `turnos` ADD `duracion` integer;--> statement-breakpoint
...
ALTER TABLE `turnos` ADD `horaLlegada` integer;--> statement-breakpoint
...
ALTER TABLE `derivaciones` ADD `pacienteId` text REFERENCES pacientes(id);--> statement-breakpoint
ALTER TABLE `diagnostico` ADD `estado` text DEFAULT 'activo' NOT NULL;--> statement-breakpoint
ALTER TABLE `diagnostico` ADD `ultimaModificacionPorId` text REFERENCES users(id);--> statement-breakpoint
ALTER TABLE `medicamentos` ADD `via` text;--> statement-breakpoint
ALTER TABLE `medicamentos` ADD `indicacion` text;--> statement-breakpoint
ALTER TABLE `notasMedicas` ADD `userMedicoId` text NOT NULL REFERENCES users(id);--> statement-breakpoint
...
ALTER TABLE `notasMedicas` DROP COLUMN `userId`;--> statement-breakpoint
ALTER TABLE `signosVitales` ADD `perimetroAbdominal` integer;--> statement-breakpoint
ALTER TABLE `signosVitales` ADD `perimetroCefalico` integer;--> statement-breakpoint
ALTER TABLE `signosVitales` ADD `presionSiscolica` integer;--> statement-breakpoint
ALTER TABLE `signosVitales` ADD `presionDiastolica` integer;--> statement-breakpoint
ALTER TABLE `signosVitales` DROP COLUMN `presionArterial`;--> statement-breakpoint
ALTER TABLE `signosVitales` DROP COLUMN `tensionArterial`;--> statement-breakpoint
ALTER TABLE `users` ADD `mp` text;--> statement-breakpoint
ALTER TABLE `users` ADD `especialidad` text;--> statement-breakpoint
ALTER TABLE `users` ADD `entidadId` text;