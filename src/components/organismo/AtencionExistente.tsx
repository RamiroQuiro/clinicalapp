import { AtencionMedicaExiste } from '@/types/index';
import {
  ArrowBigRightDash,
  Blocks,
  FileChartColumn,
...
export default function AtencionExistente({ atencionData }: AtencionMedicaExiste) {
  let edad = calcularEdad(atencionData?.pacienteData?.fNacimiento);
  console.log(atencionData);

  return (
    <div className="flex flex-col items-start gap-2 justify-normal w-ful print:fixed ">
      <div className="flex items-center  sticky top-0 py-3 bg-primary-bg-componentes justify-between w-full mb- pb-2 border-b ">
        <h2 className="text-xl font-semibold ">
          ...
          <span className="capitalize">
            {atencionData?.antecedentesData[0].nombreDoctor}{' '}
            {atencionData?.antecedentesData[0].apellidoDoctor}
          </span>
        </h2>
      ...
      <DivReact>
        <div className="w-full items-center justify-evenly gap-2  flex">
          ...
          <p className="capitalize font- tracking-tight">
            {' '}
            duración: {Number.parseFloat(atencionData?.duracionAtencion).toFixed(0) || '0:00'}{' '}
            minutos{' '}
          </p>
        </div>
      </DivReact>
      ...
      <DivReact>
        ...
        <div className="flex  items-start justify-evenly py-2 gap-4 w-full ">
          <div className=" flex gap-2 flex-col  text-sm text-muted-foreground">
            <div className="flex items-center gap-2">
              <ArrowBigRightDash size={16} />
          ...
          <div className=" flex gap-2 flex-col text-sm capitalize text-muted-foreground">
            <div className="flex items-center cpai gap-2">
              <Mail size={16} />
      ...
      <DivReact>
        ...
        <div className="flex items-start justify-between px-4 gap-3">
          <p className="border px-2 py-0.5 rounded-lg">
            Peso: {atencionData?.signosVitales?.peso}kg
          </p>
          <p className="border px-2 py-0.5 rounded-lg">
            Temperatura: {atencionData?.signosVitales?.temperatura}°
          </p>
          <p className="border px-2 py-0.5 rounded-lg">
            Frecuencia Respiratoria: {atencionData?.signosVitales?.frecuenciaRespiratoria}
          </p>
          <p className="border px-2 py-0.5 rounded-lg">
            Tension Arterial: {atencionData?.signosVitales?.tensionArterial}
          </p>
        </div>
      </DivReact>
      <DivReact>
      ...
      <DivReact>
        ...
        <ConfeccionTablaDiagnosticoHistoriaModal arrayDiagnosticos={[]} />
      </DivReact>
      {/* medicamentos */}
      <DivReact>
        <h2 className="text-lg font-semibold ">Medicamentos</h2>
        <ConfeccionTablaMedicamentosHistoriaModal arrayMedicamentos={[]} />
      </DivReact>
    </div>
  );
}
