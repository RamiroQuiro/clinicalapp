...
export default function Calendario({
  ...
  const modifiers = {
    ...
    busy: (day: Date) => dayOccupancy[formatDateToYYYYMMDD(day)]?.estado === 'busy',
    moderate: (day: Date) => dayOccupancy[formatDateToYYYYMMDD(day)]?.estado === 'moderate',
    free: (day: Date) => dayOccupancy[formatDateToYYYYMMDD(day)]?.estado === 'free',
  };

  const modifiersStyles = {
    full: { backgroundColor: '#fecaca', color: '#dc2626', border: '1px solid #dc2626' },
    busy: { backgroundColor: '#fed7aa', color: '#ea580c', border: '1px solid #ea580c' },
    moderate: { backgroundColor: '#fef08a', color: '#ca8a04', border: '1px solid #ca8a04' },
    free: { backgroundColor: '#bbf7d0', color: '#16a34a', border: '1px solid #16a34a' },
  };
  ...
  const DayContent = (props: any) => {
    ...
    if (!occupancyData)
      return <div className="relative z-10">{new Date(date).getDate()}</div>;
