...
import { setFechaYHora } from '@/context/agenda.store';
import { fechaSeleccionada, recepcionStore, setPacienteSeleccionado } from '@/context/recepcion.recepcionista.store';
...
export default function AgendaDeRecpcion({ user }: Props) {
    ...
    const turnosOcupados = useMemo(() => {
        return turnosDelDia.filter(slot => !slot.disponible).sort((a, b) => a.hora.localeCompare(b.hora));
    }, [turnosDelDia]);
