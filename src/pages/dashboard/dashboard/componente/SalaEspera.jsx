...
const SalaEspera = ({ user }) => {
  ...
  const colaDeEspera = useMemo(() => {
    ...
    return agendaDelDia.filter(
      turno =>
        turno.turnoInfo?.estado === 'sala_de_espera' || turno.turnoInfo?.estado === 'demorado'
    );
  }, [agendaDelDia]);
  useEffect(() => {
  ...
  const handleLlamarPaciente = async turno => {
    try {
      console.log(
        `Llamando a paciente ${turno.turnoInfo?.pacienteNombre} con ID ${turno.turnoInfo?.id}`
      );
    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-2 pb-2 border-b w-full text-primary-textoTitle">
        ...
        <span className="md:text-2xl">
          {turnosDelDia?.filter(
            t => t?.turnoInfo?.estado === 'sala_de_espera' || t?.turnoInfo?.estado === 'demorado'
          )?.length || 0}
        </span>
          ...
          <ul className="flex flex-col justify-normal items-start gap-2 w-full">
            {colaDeEspera?.filter(
              ...
              colaDeEspera
                ?.filter(
                  t =>
                    t?.turnoInfo?.estado === 'sala_de_espera' || t?.turnoInfo?.estado === 'demorado'
                )
                .map((turno, i) => (
                  <CardSalaEspera
                    key={i}
