...
const VitalSignChart = ({ title, history, unit, color }) => {
  ...
  // 1. Ordenar el historial por fecha de forma ascendente
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `${title} (${unit})`,
        color: '#e5e7eb', // Color de texto claro para fondo oscuro
        font: {
          size: 16,
          weight: 'bold',
        },
    ...
    scales: {
      y: {
        ...
        grid: {
          color: 'rgba(255, 255, 255, 0.1)', // Líneas de la grilla más sutiles
        },
      },
      x: {
        ticks: {
...
export const SignosVitalesPantalla = ({ signosVitalesHistorial }) => {
    ...
    <Section title="Evolución de Signos Vitales">
      {chartsToDisplay.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {chartsToDisplay.map((vital, index) => (
            <VitalSignChart
              ...
              color={chartColors[index % chartColors.length]} // 2. Asignar color variado
            />
          ))}
        </div>
      ) : (
        <div className="text-center text-gray-500 p-8 bg-gray-50 rounded-lg">
          <p>
            No hay suficiente historial (se necesitan al menos 2 registros) para mostrar gráficos.
          </p>
        </div>
      )}
    </Section>
