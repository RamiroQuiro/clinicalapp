...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const [userCentro] = await db
    ...
    .where(and(eq(usersCentrosMedicos.userId, user.id), eq(usersCentrosMedicos.activo, true)));
  ...
  try {
    ...
    const [dataProfesional] = await db.select().from(users).where(eq(users.id, newTurno.userMedicoId));
    ...
    if (newTurno.estado === 'en_sala_de_espera') {
    ...
    if (error.message.includes('UNIQUE constraint failed: turnos.fechaTurno, turnos.userMedicoId')) {
      return createResponse(409, 'Ya existe un turno para este profesional en la fecha y hora seleccionadas.');
    }
